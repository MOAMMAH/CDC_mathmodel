...
class ContraDC():
    ...
    **Class Attributes**: Are calculated by the different member functions during simulation.
    ...
    def getApodProfile(self):
        ...
        elif self.apod_shape is "uniform":
            apod = self.kappa*np.ones(self.N_seg)    
    ...
    def WvlData(self):
        """ extracts the sel.wavelength, self.thru, self.drop data and save it in txt file """  
        data = np.vstack((self.wavelength*1e9, self.thru, self.drop)).T
        #save it in results folder
        filename="results/ContraDCData_"+str(self.period)+"_"+str(self.T)+"_.txt"
        np.savetxt(filename, data, delimiter=",")
        print("Data saved in ContraDC_data.txt")
        return self  
